// Copyright Epic Games, Inc. All Rights Reserved.
/*===========================================================================
	Generated code exported from UnrealHeaderTool.
	DO NOT modify this manually! Edit the corresponding .h files instead!
===========================================================================*/

// IWYU pragma: private, include "RoadFamilyRegistry.h"

#ifdef TRAFFICRUNTIME_RoadFamilyRegistry_generated_h
#error "RoadFamilyRegistry.generated.h already included, missing '#pragma once' in RoadFamilyRegistry.h"
#endif
#define TRAFFICRUNTIME_RoadFamilyRegistry_generated_h

#include "UObject/ObjectMacros.h"
#include "UObject/ScriptMacros.h"

PRAGMA_DISABLE_DEPRECATION_WARNINGS

// ********** Begin ScriptStruct FRoadFamilyInfo ***************************************************
struct Z_Construct_UScriptStruct_FRoadFamilyInfo_Statics;
#define FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_12_GENERATED_BODY \
	friend struct ::Z_Construct_UScriptStruct_FRoadFamilyInfo_Statics; \
	static class UScriptStruct* StaticStruct();


struct FRoadFamilyInfo;
// ********** End ScriptStruct FRoadFamilyInfo *****************************************************

// ********** Begin Class URoadFamilyRegistry ******************************************************
struct Z_Construct_UClass_URoadFamilyRegistry_Statics;
TRAFFICRUNTIME_API UClass* Z_Construct_UClass_URoadFamilyRegistry_NoRegister();

#define FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_47_INCLASS_NO_PURE_DECLS \
private: \
	static void StaticRegisterNativesURoadFamilyRegistry(); \
	friend struct ::Z_Construct_UClass_URoadFamilyRegistry_Statics; \
	static UClass* GetPrivateStaticClass(); \
	friend TRAFFICRUNTIME_API UClass* ::Z_Construct_UClass_URoadFamilyRegistry_NoRegister(); \
public: \
	DECLARE_CLASS2(URoadFamilyRegistry, UObject, COMPILED_IN_FLAGS(0 | CLASS_Config), CASTCLASS_None, TEXT("/Script/TrafficRuntime"), Z_Construct_UClass_URoadFamilyRegistry_NoRegister) \
	DECLARE_SERIALIZER(URoadFamilyRegistry) \
	static constexpr const TCHAR* StaticConfigName() {return TEXT("EditorPerProjectUserSettings");} \



#define FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_47_ENHANCED_CONSTRUCTORS \
	/** Standard constructor, called after all reflected properties have been initialized */ \
	NO_API URoadFamilyRegistry(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get()); \
	/** Deleted move- and copy-constructors, should never be used */ \
	URoadFamilyRegistry(URoadFamilyRegistry&&) = delete; \
	URoadFamilyRegistry(const URoadFamilyRegistry&) = delete; \
	DECLARE_VTABLE_PTR_HELPER_CTOR(NO_API, URoadFamilyRegistry); \
	DEFINE_VTABLE_PTR_HELPER_CTOR_CALLER(URoadFamilyRegistry); \
	DEFINE_DEFAULT_OBJECT_INITIALIZER_CONSTRUCTOR_CALL(URoadFamilyRegistry) \
	NO_API virtual ~URoadFamilyRegistry();


#define FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_44_PROLOG
#define FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_47_GENERATED_BODY \
PRAGMA_DISABLE_DEPRECATION_WARNINGS \
public: \
	FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_47_INCLASS_NO_PURE_DECLS \
	FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h_47_ENHANCED_CONSTRUCTORS \
private: \
PRAGMA_ENABLE_DEPRECATION_WARNINGS


class URoadFamilyRegistry;

// ********** End Class URoadFamilyRegistry ********************************************************

#undef CURRENT_FILE_ID
#define CURRENT_FILE_ID FID_Users_rllax_L_E_O_Plugins_AAAtrafficSystem_Source_TrafficRuntime_Public_RoadFamilyRegistry_h

PRAGMA_ENABLE_DEPRECATION_WARNINGS
